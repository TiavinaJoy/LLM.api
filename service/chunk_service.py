...
from db_service import insert_chunk
...
def is_structural_element(tag):
    ...
    structural_class_patterns = [
        'container', 'wrapper', 'section', 'block', 'card', 'panel',
        'component', 'widget', 'module', 'box', 'item'
    ]
    if any(pattern in ' '.join(classes).lower() for pattern in structural_class_patterns):
        return True
...
def chunk_html(html: str, max_chunk_size: int = 2000):
    ...
    for sec in sections:
        ...
        if len(text) <= max_chunk_size:
            ...
            children = [
                child for child in sec.find_all(True, recursive=False)
            ...
            if children:
                ...
                for child in children:
                    ...
                    child_text = clean_text(child.get_text(separator=" ", strip=True))
                    if not child_text:
                        continue
